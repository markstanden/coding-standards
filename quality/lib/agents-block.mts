// lib/agents-block.mts — decision #14: seed, never own, a consumer AGENTS.md.
//
// Writes a marker-delimited managed block (<!-- quality-gate:start --> …
// <!-- quality-gate:end -->) into a target AGENTS.md from a template. Re-runs
...
export const BLOCK_START = "<!-- quality-gate:start -->";
export const BLOCK_END = "<!-- quality-gate:end -->";
...
export async function writeMarkedBlock({
  ...
  if ((startLine === -1) !== (endLine === -1) || (startLine !== -1 && startLine > endLine)) {
    throw new Error(
      `${filePath} has a half-written or out-of-order quality-gate block — resolve by hand, do not rerun`,
    );
  }
