...
// Run: node --test quality/lib/git.test.mts
...
async function makeRepo({
  ...
  for (const rel of [".gitignore", ...(committed ?? []), ...(untracked ?? []), ...(ignored ?? [])]) {
...
test("trackedFiles lists tracked and untracked-but-not-ignored files", async () => {
  ...
  assert.deepEqual((await Promise.resolve(trackedFiles({ repoRoot: root }))).sort(), [".gitignore", "a.sh", "b.sh"]);
});

test("trackedFiles excludes ignored and untracked-but-ignored files", async () => {
  const root = await makeRepo({ committed: ["a.sh"], ignored: ["gen.ignored"] });
  assert.deepEqual(trackedFiles({ repoRoot: root }).sort(), [".gitignore", "a.sh"]);
});
