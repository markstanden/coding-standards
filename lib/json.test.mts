...
test("readJsonFile parses a valid file", async () => {
  ...
  try {
    ...
    assert.deepEqual(await readJsonFile<{ a: number }>({ filePath: file }), { a: 1 });
  } finally {
...
test("readJsonFile throws naming the missing file", async () => {
  const dir = await makeTemp();
  try {
    await assert.rejects(() => readJsonFile({ filePath: join(dir, "absent.json") }), /not found/u);
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
...
test("readJsonFile throws naming invalid JSON", async () => {
  ...
  try {
    ...
    await assert.rejects(() => readJsonFile({ filePath: file }), /invalid JSON/u);
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
