...
test("returns 0 on no changes and 2 on changes detected", () => {
  assert.equal(runTofuPlan({ infraDir: "/repo/infra", runner: scriptedRunner({}).runner }), 0);
  assert.equal(runTofuPlan({ infraDir: "/repo/infra", runner: scriptedRunner({ "tofu plan -detailed-exitcode -no-color -out=tfplan": { status: 2 } }).runner }), 2);
});

test("returns 1 (error) as-is for the caller to gate on", () => {
  assert.equal(runTofuPlan({ runner: scriptedRunner({ "tofu plan -detailed-exitcode -no-color -out=tfplan": { status: 1, stdout: "boom" } }).runner }), 1);
});
