...
test("throws when tofu init fails", () => {
    const { runner } = scriptedRunner({
        "tofu init -reconfigure -input=false -backend-config=resource_group_name=rg -backend-config=storage_account_name=sa -backend-config=container_name=container -backend-config=key=key":
...
test("throws when both select and new fail", () => {
    ...
    assert.throws(
        () => runTofuInit({ ...baseInput, buildEnv: "dev", runner }),
        /tofu workspace select\|new failed:\nno create/u,
    );
});
...
test("clears a stale .terraform/environment marker before init", async () => {
    ...
    try {
        ...
        assert.deepEqual(calls[0], [
            "rm",
            "-f",
            join(dir, ".terraform", "environment"),
        ]);
    } finally {
