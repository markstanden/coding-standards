...
test("reads the version from packages.lock.json", async () => {
  ...
  try {
    await writeFile(
      ...
      JSON.stringify({
        dependencies: { "Microsoft.Playwright": { resolved: "1.44.0", requested: "[1.44.0, )" } },
      }),
    );
    assert.equal(await detectPlaywrightVersion({ projectDir: dir }), "1.44.0");
  } finally {
...
test("falls back to project.assets.json when no lock file version", async () => {
  ...
  try {
    ...
    await writeFile(
      ...
      JSON.stringify({
        libraries: { "Microsoft.Playwright/1.45.1": { type: "package" } },
      }),
    );
    assert.equal(await detectPlaywrightVersion({ projectDir: dir }), "1.45.1");
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
