...
export async function detectPlaywrightVersion({
    projectDir,
}: PlaywrightVersionInput): Promise<string> {
    ...
    if (lock !== "") {
        try {
            const data = JSON.parse(lock) as {
                dependencies?: Record<
                    string,
                    Record<string, { resolved?: string; version?: string }>
                >;
            };
    ...
    if (assetsPath) {
        try {
            ...
            const lib = Object.keys(data.libraries ?? {})
                .filter((k) =>
                    k.toLowerCase().startsWith("microsoft.playwright/"),
                )
        ...
        for (const entry of entries) {
            ...
            if (entry.name === "obj") {
                const exists = await readFile(
                    join(candidate, "project.assets.json"),
                    "utf8",
                )
                    .then(() => true)
                    .catch(() => false);
